...
class TensorDataset:
    def __init__(self, path, img_width, img_height, split: str, aug=False):
        ...
        #         data_path = (
        #             f"/home/eduard/Github/datasets/coco/{line.strip().split('./')[-1]}"
        #         )
        ...
        # _, path = foz.download_zoo_dataset(f"dataset/coco-2017/{split}", split=split)

        dataset = foz.load_zoo_dataset(
            "coco-2017", max_samples=128, split=split, label_types=["detections"]
        )
        dataset.export(
            export_dir="dataset",
        ...
        print(f"dataset path: {split}")

        self.data_list = [
            os.path.join(f"dataset/images/{split}", img)
            for img in os.listdir(f"dataset/images/{split}")
        ]
